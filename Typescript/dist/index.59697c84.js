...
function calculate() {
    ...
    for(i in itemList){
        ...
        else number = getCalculation(number, Number(itemList[i]), sign);
    }
    // Round Up Number
    number = Math.round(number * 100) / 100;
    if (number.toString().length > 12) number = "Err: Num Too Big";
...
function setNumber() {
    ...
    if (CALCULATOR_STORE.length === 0) CALCULATOR_STORE += this.name;
    else {
        let lastIdx = CALCULATOR_STORE.length - 1;
        let isNumber = /[0-9.]+/.test(CALCULATOR_STORE[lastIdx]);
        if (isNumber) CALCULATOR_STORE += this.name;
        else CALCULATOR_STORE += ` ${this.name}`;
    }
    updateDisplay();
}
function setSigns() {
    ...
    let isNumber = /[0-9.]+/.test(CALCULATOR_STORE[lastIdx]);
    if (isNumber) CALCULATOR_STORE += ` ${this.name}`;
    else CALCULATOR_STORE = CALCULATOR_STORE.slice(0, lastIdx) + this.name;
    updateDisplay();
}
function setDot() {
    ...
    if (isDotlessNumber) CALCULATOR_STORE = CALCULATOR_STORE + this.name;
    updateDisplay();
}
...
function clearDisplay() {
    if (CALCULATOR_STORE.length === 0) updateDisplay(0);
    else {
        let lastIdx = CALCULATOR_STORE.length - 1;
        let isNumber = /[0-9.]+/.test(CALCULATOR_STORE[lastIdx]);
        // Slice to last index when number, if sign then slice one more
        if (isNumber) CALCULATOR_STORE = CALCULATOR_STORE.slice(0, lastIdx);
        else CALCULATOR_STORE = CALCULATOR_STORE.slice(0, lastIdx - 1);
